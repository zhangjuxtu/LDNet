...
class Generator(object):
    ...
    def Generate(self): 
        ...
        for start, end in cuts:
            ...
            #self.inputs, self.target_lanes, self.target_h, self.test_image, self.meta = self.Resize_data(start, end)
            self.inputs, self.target_lanes, self.target_h = self.Resize_data(start, end)
            ...
            #yield self.inputs, self.target_lanes, self.target_h, self.test_image, self.meta  # generate normalized image
            yield self.inputs, self.target_lanes, self.target_h  # generate normalized image
    ...
    def Generate_Test(self): 
        for i in range(self.size_test):
            ...
            lanes = self.test_data[i]['lanes']
            h_samples = np.array(self.test_data[i]['h_samples'])
            ...
            _, meta = util.make_ground_truth_point(test_lanes, test_h, self.p)
            meta.update({'gt_image':np.array([gt_image]), 'ratio_w':ratio_w, 'ratio_h':ratio_h, 'index':i})
 
            yield test_image, h_samples, meta 

    #################################################################################################################
    ## Generate data as much as batchsize for fast test
    #################################################################################################################
    def Generate_test_batch(self): 
        ...
        for start, end in cuts:
            ...
            test_image, path, ratio_w, ratio_h, targeth,  target_h, target_lanes = self.Resize_data_test(start, end)
            _, meta = util.make_ground_truth_point(target_lanes, target_h, self.p)

            yield test_image/255.0, targeth, ratio_w, ratio_h, path, meta
    ...
    def Resize_data_test(self, start, end):
        ...
        for i in range(start, end):
            ...
            targeth.append( np.array(data['h_samples']) )
        ...
        return np.array(inputs), path, ratio_w, ratio_h, targeth, target_h, target_lanes
    ...
    def Resize_data(self, start, end):
        ...
        ratio_h = self.p.y_size*1.0/test_image.shape[0]
        test_image = cv2.resize(test_image, (self.p.x_size,self.p.y_size))
        gt_test_image = deepcopy(test_image)
        test_image = np.rollaxis(test_image, axis=2, start=0)

        lanes = self.test_data[test_index]['lanes']
        hsample = self.test_data[test_index]['h_samples']
        temp_lanes = []
        temp_h = []
        for j in lanes:
            temp = np.array(j)
            temp = temp*ratio_w
            temp_lanes.append( temp )
            temp_h.append( np.array(hsample)*ratio_h )
        test_lanes = [np.array(temp_lanes)]
        test_h = [np.array(temp_h)]
        _, meta = util.make_ground_truth_point(test_lanes, test_h, self.p)
        gt_image = util.draw_lanes(gt_test_image, lanes, hsample, ratio_w, ratio_h)
        meta.update({'gt_image':np.array([gt_image]), 'ratio_w':ratio_w, 'ratio_h':ratio_h})
        '''
        
        #return np.array(inputs), target_lanes, target_h, test_image, meta
