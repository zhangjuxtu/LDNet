...
class Generator(object):
    ...
    def lane_process(self, lanes):
        ...
        for i, x in enumerate(lanes):
            ...
            if len(start_val) == 0:
                start_val.append(tmp_start_val)
                tmp_lanes.append(x)
            else:
                for j, val in enumerate(start_val):
                    if tmp_start_val < val:
                        start_val.insert(j, tmp_start_val)
                        tmp_lanes.insert(j, x)
                        break
                    if j == len(start_val) - 1:
                        tmp_lanes.append(x)
        return tmp_lanes
    ...
    def split(self):
        ...
        for i in range(self.size_train):
            # process the source lane data
            self.train_data[i]['lanes'] = self.lane_process(self.train_data[i]['lanes'])
